...
if (!defined('TMDB_API_KEY') || TMDB_API_KEY === 'TU_API_KEY_AQUI' || empty(TMDB_API_KEY)) {
    echo json_encode(["error" => "La clave API de TMDB no está configurada."]);
    exit;
}
...
if (!$movieData) {
    echo json_encode(["error" => "No se pudo obtener información detallada de la película desde TMDB."]);
    exit;
}
...
if ($titulo === '' || $release_date === '') {
    echo json_encode(["error" => "La película seleccionada carece de título o fecha de estreno obligatorios."]);
    exit;
}
...
if (mysqli_num_rows($resCheck) > 0) {
    echo json_encode(["error" => "La película '$titulo' ($release_date) ya existe en tu catálogo local."]);
...
try {
    ...
    // 5. Procesar Reparto (Top 5 actores)
    if (!empty($movieData['credits']['cast'])) {
        ...
        foreach ($topCast as $castMember) {
            ...
            // Asociar actor a película con personaje
    ...
    $errorMsgTransaccion = $e->getMessage();
}
...
if ($exitoTransaccion) {
    ...
    echo json_encode(["error" => "Error durante el guardado en base de datos: " . $errorMsgTransaccion]);
}
